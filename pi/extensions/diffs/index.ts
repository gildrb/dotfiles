...
function renderDiff(
	...
	while (index < rows.length) {
		...
		const number = formatNumber(
			row.kind === "remove" ? row.oldLine : (row.newLine ?? row.oldLine),
			width,
			theme,
		);
...
function previewDiff(state: unknown): string | undefined {
	...
	const preview = (state as { preview?: { diff?: string; error?: string } })
		.preview;
	if (!preview || preview.error || typeof preview.diff !== "string")
		return undefined;
	return preview.diff;
}
...
function resultDiff(result: {
	details?: unknown;
}): { diff: string; startLine: number } | undefined {
	...
	if (typeof record.diff !== "string" || record.diff.length === 0)
		return undefined;
	const first =
		typeof record.firstChangedLine === "number" ? record.firstChangedLine : 1;
