...
const DIFF_TOOLS = new Set(["edit", "replace", "undo_last_replace", "write"]);
...
function renderNumbered(content: string, filePath: string | undefined, theme: Theme): string {
  const lang = filePath ? getLanguageFromPath(filePath) : undefined;
  const source = content.replace(/\t/g, "  ");
  const highlighted = lang ? highlightCode(source, lang) : source.split("\n").map((line) => theme.fg("mdCodeBlock", line));
  const width = String(Math.max(highlighted.length, 1)).length;
  return highlighted
    .map((line, index) => `${theme.fg("dim", `${String(index + 1).padStart(width, " ")}→`)}${line}`)
    .join("\n");
}
  ...
  tool.renderCall = (args, theme, context) => {
    ...
    if (tool.name === "write") {
      ...
      if (typeof content === "string" && content.length > 0) {
        const numbered = renderNumbered(content, path, theme).split("\n");
        const shown = context.expanded ? numbered : numbered.slice(0, 12);
        const extra =
          !context.expanded && numbered.length > shown.length
            ? `\n${theme.fg("dim", `… ${numbered.length - shown.length} more lines`)}`
            : "";
        const header = `${theme.fg("toolTitle", theme.bold("write"))} ${theme.fg("mdLink", path ?? "")}`;
        return setComponentText(rendered, `${header}\n${shown.join("\n")}${extra}`) as Text;
      }
      return rendered ?? new Text("", 0, 0);
    }
