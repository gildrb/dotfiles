...
- Report prices only in EUR. Do not show USD prices, exchange rates, or conversion calculations.
...
12. Compute price from the token counts and models.dev rates. Be careful about units: most prices are per 1M tokens. Account for separate input, output, and cached read/input rates when models.dev provides them. If a rate is unavailable, use 0 for that component and add a note. Present the resulting price only as EUR.
13. Present the result as concise Markdown:
   ...
   - Format token counts with commas and EUR with 4 decimal places unless larger amounts warrant 2 decimals.
14. Add a short "Pricing notes" section listing models.dev lookup date, unmatched models, assumptions, Codex CLI parsing assumptions, and skipped/invalid session lines if any. Do not include exchange-rate notes.
...
- For models.dev pricing, prefer a script that downloads/parses/filter-matches outside agent context and prints only compact JSON or table rows for relevant models. Avoid tool calls that return the complete api.json markdown/content to the agent. Do not modify any session files.`;

export default function (pi: ExtensionAPI) {
  pi.registerCommand("usage", {
    description:
      "Ask the agent to summarize Pi and Codex CLI usage and cost in EUR for the last 1, 7, 30, and 90 days",
