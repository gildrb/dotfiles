...
In this Pi setup, the installed script is available at:

```bash
export SENTRY_API="$HOME/.pi/agent/vendor/sentry/skills/sentry/scripts/sentry_api.py"
```

If you are running from another installed plugin copy instead of this Pi copy, use the same
`skills/sentry/scripts/sentry_api.py` path inside that installed plugin directory.
